...
class LibreriaPoo(QMainWindow):
    ...
    def __configurar(self):
        ...
        tableModel.setHorizontalHeaderLabels(["DNI", "Nombre"])
        self.tableView_prestamos.setModel(tableModel)
        self.tableView_prestamos.setEditTriggers(QAbstractItemView.NoEditTriggers)
        self.tableView_prestamos.setColumnWidth(0, 230)
        self.tableView_prestamos.setColumnWidth(1, 65)
        ...
        self.btn_prestamo.clicked.connect(self.agregar_prestamo)
    ...
    def agregar_prestamo(self):
        nombre, ok_pressed = QInputDialog.getInt(self, "Agregar prestamo", "numero de prestamos")
        if ok_pressed:
            model = self.lisView_usuarios.mode()
            value = model.itemFromIndex(self.listView_usuarios.selectedIndexes([0]))
            self.libreria.agregar_a_prestados(value.usuario, nombre)

    def agregar_usuario_tabla(self, item):
        celda_1 = QStandardItem(item.usuario.dni)
        celda_2 = QStandardItem(str(item.usuario.nombre))
        celda_1.item = item
        model = self.tableView_prestamos.model()
        model.appendRow([celda_1, celda_2])
